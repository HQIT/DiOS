...
DiOS 是 DiFlow 工具链中 Agent 的**环境支持层**，职责类似传统操作系统：
...
**DiOS 不负责编排**——Agent 之间的协作逻辑由 Agent 自身完成（通过 MCP/HTTP 互相调用），DiOS 只负责把外部事件正确地路由给 Agent。

### 1.2 核心概念

- **一个 Agent = 一个 DiAgent 容器**，独立运行，常驻待命
- Agent 通过 MCP/HTTP 与外部系统和其他 Agent 交互
- DiOS 是基础设施，不介入 Agent 的业务逻辑

### 1.3 为什么需要 Event Gateway

在多智能体协作场景中，Agent 需要被外部事件触发（Git webhook、邮件、Slack 消息等）。Event Gateway 是 DiOS 的"中断/信号"机制——接收外部 I/O 事件并路由给对应的 Agent 进程。
...
- DiOS 负责校验签名（如 GitHub webhook secret）并标准化
...
| 配置项 | 来源 | 说明 |
|--------|------|------|
| 模型配置 | `configs/models.yaml` (volume mount) | DiOS 生成并挂载 |
| MCP 配置 | `configs/mcp_servers.json` (volume mount) | DiOS 生成并挂载 |
| Skills | `workspace/skills/` (volume mount) | DiOS 管理 |
| 系统提示词 | 环境变量 `AGENT_SYSTEM_PROMPT` 或配置文件 | DiOS 注入 |
| 默认模型 | 环境变量 `LLM_DEFAULT_MODEL` | DiOS 注入 |
| 端口 | 环境变量 `PORT`，默认 8000 | DiOS 分配 |
...
Connector 负责将外部事件接入 DiOS Event Gateway，产生 CloudEvent 并匹配订阅、触发 Agent。
...
- 收邮件：配置 IMAP 类型的 Connector，DiOS 后台轮询拉取新邮件并生成事件。
