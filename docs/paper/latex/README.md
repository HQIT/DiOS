...
最新可交付文件为 `E2AG-paper-anonymous-v1.1.6-20260815.pdf`。v1.1.6 是不改变正文和实验数字的配图修订版：图 2 保留状态胶囊的水平侧入空心箭头，同时利用加高后的执行平面将拒绝、等待审批和调用拒绝分支均简化为一次圆角转弯；其余版式沿用 v1.1.4 的统一网格。
