...
考虑一个具有合法 CloudEvent 结构的 Git 推送事件。不可信来源可能给出与其身份不一致的来源字段，也可能使任务在创建后选择 grant 集合之外的工具。入口模式校验只能确认字段存在，目标智能体的静态工具配置不能表达“本次事件是否有权创建该任务”，而调度时已经允许的任务也可能在运行中选择另一工具。若工具网关只看到工具名而看不到触发事件和任务身份，它同样无法判断一个表面合法的调用是否具有完整授权。上述失效并非同一规则的重复遗漏，而是授权对象在事件、任务和工具三个信任域之间发生了变化。
...
标签复核 & 3名复核者$\times$60例 & 检查规则可复核性，不证明分布代表性 \\
...
正式冻结矩阵包含 GitHub、GitLab、Gitea、IMAP、通用 Webhook、Manual 和 Cron。30个违规样本由来源--类型绑定不一致、目标来源/类型/动作/工具集合外请求和需审批事件组成；所有样本满足基本 CloudEvent 结构，以避免把解析失败计为跨层治理收益。三名复核者仅接收移除作者标签的事件字段与目标治理声明，分别标注事件类别、首个治理层和完整 E2AG 预期决策。报告违规拒绝率、正常通过率、Wilson 95\% 置信区间以及多复核者 Fleiss $\kappa$。
...
表~\ref{tab:rq1-v11} 显示 ContractGuard 只阻断来源--类型绑定违规，违规拒绝率置信区间为[19.23\%, 51.22\%]；当来源具有合法契约但目标智能体不允许其动作或工具时，契约不产生作用。DispatchGuard 阻断目标能力不匹配并将需审批动作转入审批，区间为[48.78\%, 80.77\%]，但无法识别来源绑定不匹配。完整组合覆盖两类互不重叠的拒绝原因，区间为[88.65\%, 100\%]。四组正常通过率相同，说明差异来自治理对象而不是普遍收紧接入。

三名复核者在事件类别、首个治理层和完整 E2AG 预期决策三项上均达到60/60逐例全体一致，并与冻结作者标签完全一致；三项 Fleiss $\kappa$ 均为1.00。三份表的冻结输入字段均未被修改，且逐例备注在任意两名复核者间没有完全相同项。该结果支持规则标签明确且可由非构造者复核，不说明矩阵覆盖自然流量中的违规分布。
