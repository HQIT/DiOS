...
该配对结果保留了真实 qame--demogo--DiAgent--PEP--\texttt{git-perf} 路径，同时把负向判断收缩为可复核的治理一致性性质：集合内调用能够到达上游，集合外调用由 PEP 本地拒绝且不改变上游状态。它不要求模型生成或推进越权步骤，也不被解释为模型安全率、攻击成功率、吞吐量或外部方法比较。
