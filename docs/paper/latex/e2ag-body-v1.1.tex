...
\begin{enumerate}
  ...
  \item 在真实事件驱动智能体操作系统原型中实现 E2AG，并通过完整消融、持久化端到端执行、既有模型决策配对回放、故障注入和并发状态实验验证各机制的独立作用与组合效果；
...
表~\ref{tab:related-scope-v11} 按保护对象、执行点和证据类型比较代表性方法。协议标准不承担治理，现有运行时方法集中于动作、任务意图或数据流；E2AG 同时约束事件声明、系统任务创建和工具副作用，并把三个判定关联到同一执行证据。由于这些方法的输入主体和保护对象不同，本文只进行机制范围比较，数值实验采用共享代码路径的治理位置基准，避免把不同系统的公开结果直接排序。
...
外部策略引擎 & OPA v1.17.0；8个冻结\texttt{tools/call}用例 & 工具边界功能比较，不报告本地时延排名 \\
...
首先在\texttt{tool\_call\_cases.jsonl}中选取8个\texttt{tools/call}请求，其中4个工具名属于任务允许集合，4个在集合外。OPA-Tool 与 RuntimeGuard 接收相同的3项工具模式；Rego 策略只规定“方法为\texttt{tools/call}且工具名匹配允许模式时通过”，不调用 E2AG 判定代码。原冻结集另有2个非\texttt{tools/call}协议方法，它们检验 MCP 方法中介而非工具授权，故不计入 OPA-Tool 分母。

\begin{table}[htbp]
\centering
\caption{冻结工具调用切片上的独立策略引擎基线}
...
表~\ref{tab:rq2-opa-v11} 表明任务级工具允许集的判定语义可由独立通用策略引擎复现，RuntimeGuard 的结果不是由内部测试桩特有行为造成。该结果不表示 E2AG 在工具名判定上优于 OPA；二者在此切片功能等价。差异在保护范围：OPA-Tool 没有接收事件来源、任务准入、能力终态或跨层证据，因而不能回答同一工具调用是否继承自合法事件和有效任务。实验记录固定 OPA 版本、发布件 SHA-256、Rego 策略哈希、输入哈希和逐例结果，不报告单机进程启动时延。
...
\textbf{内部有效性。}冻结矩阵按治理层构造，适合验证机制分工，不用于估计自然流量中的违规发生率。三名非构造者的盲表复核排除了明显的标签歧义，但因复核者依据同一显式治理规则判定，完全一致不能证明语料具有自然流量代表性。既有模型结果采用固定提示和工具选择约束，保证相同决策可在不同治理配置下配对复用；它证明副作用可达性，不证明开放环境中的模型违规选择概率。OPA-Tool 提供独立策略引擎的同输入功能参照，但 Rego 策略由本文按公开接口适配，只覆盖工具边界，不能替代 AgentSpec、OAP、ToolGuardian 或 Agent libOS 的原系统复现。
...
本文针对事件驱动 AIOS 中事件来源、任务创建和工具副作用跨越不同信任域的问题，提出跨层能力治理方法 E2AG。该方法用来源--类型契约建立事件声明权，用目标策略和审批控制任务创建，用任务作用域能力与调用时 PEP 约束模型最终选择的工具，并通过统一执行证据连接各阶段对象。状态迁移模型和完整中介命题说明两个执行点为何缺一不可；消融、端到端配对执行、既有模型决策回放、故障注入与并发实验分别验证任务来源闭包、副作用安全、证据连续性和状态单调性。E2AG 的核心价值不是判定模型语言层输入类别，而是把非确定性模型决策置于可执行、可审计且对象绑定的系统授权边界内。
