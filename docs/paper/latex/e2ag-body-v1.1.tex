...
\textbf{内部有效性。}冻结矩阵按治理层构造，适合验证机制分工，不用于估计自然流量中的违规发生率。三名非构造者的盲表复核排除了明显的标签歧义，但因复核者依据同一显式治理规则判定，完全一致不能证明语料具有自然流量代表性。既有模型结果采用固定提示和工具选择约束，保证相同决策可在不同治理配置下配对复用；它证明副作用可达性，不证明开放环境中的模型违规选择概率。内部基准共享代码路径以隔离治理位置，但不能替代 AgentSpec、OAP、ToolGuardian 或 Agent libOS 的原实现比较。
