...
\node[component] (event) at (1.35,0)
  {标准化事件 $e$};
\node[proposed,minimum width=29mm] (admit) at (4.35,0)
  {调度前 PEP\\契约判定 $+$ 目标策略};
\node[component,minimum width=25mm] (task) at (7.50,0)
  {A2A Task $t$\\任务能力 $g$};
\node[proposed,minimum width=29mm] (authorize) at (10.70,0)
  {调用时 PEP\\状态 $+$ 绑定 $+$ 工具};
\node[component] (effect) at (14.00,0)
  {工具副作用 $o$};

% Conditional and terminal states are compact capsules, not competing boxes.
\node[outcome] (deny) at (3.25,1.50) {拒绝终止};
\node[outcome] (approval) at (6.00,1.50) {等待审批};
\node[outcome] (denycall) at (10.70,1.50) {调用拒绝};
...
\draw[control] ($(admit.north)+(-7mm,0)$)
  -- ($(admit.north)+(-7mm,4mm)$)
  -- (deny.south |- {$(admit.north)+(-7mm,4mm)$})
  -- (deny.south);
\draw[control] ($(admit.north)+(7mm,0)$)
  -- ($(admit.north)+(7mm,4mm)$)
  -- (approval.south |- {$(admit.north)+(7mm,4mm)$})
  -- (approval.south);
\draw[control] (approval.east)
  ...
  -- node[right,font=\scriptsize,fill=white,inner sep=1pt]{审批通过}
     ($(task.north)+(0,4mm)$)
  -- (task.north);
\draw[control] (authorize.north)--(denycall.south);

% Evidence writes are visually secondary and terminate at one complete trace.
\node[audit,minimum width=145mm] (auditlog) at (7.75,-1.55)
