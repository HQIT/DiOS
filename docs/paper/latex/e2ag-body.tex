...
\begin{tikzpicture}[
  ...
  evidence/.style={-{Latex[length=1.5mm]},draw=black,densely dotted,line width=.65pt,
    rounded corners=2pt,line join=round,line cap=round},
  boundary/.style={draw=black,dash dot,line width=.65pt}
]
...
\draw[flow] (connector.east) |- (gateway.west);
\draw[flow] (gateway)--(pep1)--(dispatcher);
\draw[flow] (dispatcher.east) |- (task.west);
\draw[flow] (task)--(runtime);
\draw[flow] (runtime.east) |- (grant.west);
...
\draw[boundary] ($(domain1.north east)!0.5!(domain2.north west)$)
  -- ($(domain1.south east)!0.5!(domain2.south west)$);
\draw[boundary] ($(domain2.north east)!0.5!(domain3.north west)$)
  -- ($(domain2.south east)!0.5!(domain3.south west)$);
\draw[boundary] ($(domain3.north east)!0.5!(domain4.north west)$)
  -- ($(domain3.south east)!0.5!(domain4.south west)$);
...
图中单线框是当前 DiOS/DiAgent 基线，双线框是 E2AG 新增或强化的治理机制，虚线框表示 Roadmap 或本文尚未统一中介的路径，点划线 TB1--TB3 标识三处信任边界，点线表示审计证据汇聚。全部语义由形状和线型编码，不依赖颜色。该图只说明论文相关的系统上下文，不把 DiOS 管理界面、模型服务和全部应用层能力列为本文贡献。
