...
| 外部策略引擎 | OPA v1.17.0；8个冻结`tools/call`用例 | 工具边界功能比较，不报告本地时延排名 |
...
原实现的弱化复现。为增加独立实现参照，本文另以官方 OPA v1.17.0 执行通用 default-deny Rego 工具允许集策略（OPA-Tool）。该基线只接收工具调用对象，因此仅与 RuntimeGuard 在相同冻结工具调用切片上比较判定结果，不比较本地时延，也不把通用策略引擎等同于事件驱动 Agent 治理系统。其他外部方法仍只在表 <a href="#tab:related-scope-v11" data-reference-type="ref"
data-reference="tab:related-scope-v11">1</a>
按保护对象比较，不对异构公开结果排序。
...
首先在 `tool_call_cases.jsonl` 中选取8个 `tools/call` 请求，其中4个工具名属于任务允许集合，4个在集合外。OPA-Tool 与 RuntimeGuard 接收相同的3项工具模式；Rego 策略只规定“方法为 `tools/call` 且工具名匹配允许模式时通过”，不调用 E2AG 判定代码。原冻结集另有2个非 `tools/call` 协议方法，它们检验 MCP 方法中介而非工具授权，故不计入 OPA-Tool 分母。
...
该结果表明任务级工具允许集的判定语义可由独立通用策略引擎复现，RuntimeGuard 的结果不是内部测试桩特有行为。它不表示 E2AG 在工具名判定上优于 OPA；二者在此切片功能等价。差异在保护范围：OPA-Tool 没有接收事件来源、任务准入、能力终态或跨层证据，因而不能回答同一工具调用是否继承自合法事件和有效任务。实验记录固定 OPA 版本、发布件 SHA-256、Rego 策略哈希、输入哈希和逐例结果，不报告单机进程启动时延。
...
**内部有效性。**冻结矩阵按治理层构造，适合验证机制分工，不用于估计自然流量中的违规发生率。三名非构造者的盲表复核排除了明显的标签歧义，但因复核者依据同一显式治理规则判定，完全一致不能证明语料具有自然流量代表性。既有模型结果采用固定提示和工具选择约束，保证相同决策可在不同治理配置下配对复用；它证明副作用可达性，不证明开放环境中的模型违规选择概率。OPA-Tool 提供独立策略引擎的同输入功能参照，但 Rego 策略由本文按公开接口适配，只覆盖工具边界，不能替代 AgentSpec、OAP、ToolGuardian 或 Agent libOS 的原系统复现。
