...
分别定义事件、任务和工具调用对象，但格式互操作不等于授权传播。策略决策点（policy decision point，PDP）与 PEP
分离支持在执行点实施结构化策略；参考监控器和完整中介原则要求所有安全相关访问经过不可绕过且足够小的检查机制；能力系统把访问权封装为可传递、可收缩的对象。万维网联盟（World Wide Web Consortium，W3C）的
