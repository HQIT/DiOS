...
该脚本首先按结构规则从60例冻结矩阵选取49例可调用切片：目标非空、事件显式声明请求工具，且目标策略含工具允许集合。全部11个排除编号和原因写入机器结果。NoGuard、E2AG 对照按相同49个编号从已有 C0P0、C1P1 冻结逐例结果投影。脚本同时保留8个 `tools/call` 接口补充用例，另2个非工具调用协议方法明确排除。输出包含逐例判定、选择规则、OPA版本和所有关键哈希，不采集本地进程启动时延。
